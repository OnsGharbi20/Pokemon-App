...
var SingleBoxSharedResizeObserver = class {
  constructor(_box) {
    ...
    this._resizeSubject = new import_rxjs4.Subject();
  ...
  observe(target) {
    if (!this._elementObservables.has(target)) {
      this._elementObservables.set(target, new import_rxjs4.Observable((observer) => {
        ...
        (0, import_operators3.filter)((entries) => entries.some((entry) => entry.target === target)),
        // Share a replay of the last event so that subsequent calls to observe the same element
        // receive initial sizing info like the first one. Also enable ref counting so the
        // element will be automatically unobserved when there are no more subscriptions.
        (0, import_operators3.shareReplay)({
          bufferSize: 1,
          refCount: true
        }),
...
var _MatFormFieldFloatingLabel = class _MatFormFieldFloatingLabel {
  ...
  constructor(_elementRef) {
    ...
    this._resizeSubscription = new import_rxjs5.Subscription();
  }
  ngOnDestroy() {
    this._resizeSubscription.unsubscribe();
  }
...
var _MatFormField = class _MatFormField {
  ...
  _initializeControl() {
    ...
    if (control.ngControl && control.ngControl.valueChanges) {
      control.ngControl.valueChanges.pipe((0, import_operators4.takeUntil)(this._destroyed)).subscribe(() => this._changeDetectorRef.markForCheck());
    }
  }
  ...
  _initializeOutlineLabelOffsetSubscriptions() {
    ...
    this._dir.change.pipe((0, import_operators4.takeUntil)(this._destroyed)).subscribe(() => this._needsOutlineLabelOffsetUpdate = true);
  }
...
var _MatSelect = class _MatSelect {
  ...
  constructor(_viewportRuler, _changeDetectorRef, _unusedNgZone, defaultErrorStateMatcher, _elementRef, _dir, parentForm, parentFormGroup, _parentFormField, ngControl, tabIndex, scrollStrategyFactory, _liveAnnouncer, _defaultOptions) {
    ...
    this._destroy = new import_rxjs6.Subject();
    this.stateChanges = new import_rxjs6.Subject();
    ...
    this._panelDoneAnimatingStream = new import_rxjs6.Subject();
    ...
    this._initialized = new import_rxjs6.Subject();
    this.optionSelectionChanges = (0, import_rxjs6.defer)(() => {
      const options = this.options;
      if (options) {
        return options.changes.pipe((0, import_operators5.startWith)(options), (0, import_operators5.switchMap)(() => (0, import_rxjs6.merge)(...options.map((option) => option.onSelectionChange))));
      }
      return this._initialized.pipe((0, import_operators5.switchMap)(() => this.optionSelectionChanges));
    });
    this.openedChange = new EventEmitter();
    this._openedStream = this.openedChange.pipe((0, import_operators5.filter)((o) => o), (0, import_operators5.map)(() => {
    }));
    this._closedStream = this.openedChange.pipe((0, import_operators5.filter)((o) => !o), (0, import_operators5.map)(() => {
    }));
  ...
  ngOnInit() {
    ...
    this.stateChanges.next();
    this._panelDoneAnimatingStream.pipe((0, import_operators5.distinctUntilChanged)(), (0, import_operators5.takeUntil)(this._destroy)).subscribe(() => this._panelDoneAnimating(this.panelOpen));
    this._viewportRuler.change().pipe((0, import_operators5.takeUntil)(this._destroy)).subscribe(() => {
      if (this.panelOpen) {
  ...
  ngAfterContentInit() {
    ...
    this._selectionModel.changed.pipe((0, import_operators5.takeUntil)(this._destroy)).subscribe((event) => {
      event.added.forEach((option) => option.select());
      event.removed.forEach((option) => option.deselect());
    });
    this.options.changes.pipe((0, import_operators5.startWith)(null), (0, import_operators5.takeUntil)(this._destroy)).subscribe(() => {
      this._resetOptions();
      this._initializeSelection();
    });
  }
   ...
   */
  _onAttached() {
    this._overlayDir.positionChange.pipe((0, import_operators5.take)(1)).subscribe(() => {
      this._changeDetectorRef.detectChanges();
  ...
  _resetOptions() {
    ...
    this.optionSelectionChanges.pipe((0, import_operators5.takeUntil)(changedOrDestroyed)).subscribe((event) => {
    ...
    (0, import_rxjs6.merge)(...this.options.map((option) => option._stateChanges)).pipe((0, import_operators5.takeUntil)(changedOrDestroyed)).subscribe(() => {
      this._changeDetectorRef.detectChanges();
      this.stateChanges.next();
    });
  }
...
var _MatTooltip = class _MatTooltip {
  ...
  constructor(_overlay, _elementRef, _scrollDispatcher, _viewContainerRef, _ngZone, _platform, _ariaDescriber, _focusMonitor, scrollStrategy, _dir, _defaultOptions, _document) {
    ...
    _dir.change.pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe(() => {
      if (this._overlayRef) {
        this._updatePosition(this._overlayRef);
      }
    });
  ...
  ngAfterViewInit() {
    ...
    this._setupPointerEnterEventsIfNeeded();
    this._focusMonitor.monitor(this._elementRef).pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe((origin) => {
      if (!origin) {
  ...
  show(delay = this.showDelay, origin) {
    ...
    instance.afterHidden().pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe(() => this._detach());
    this._setTooltipClass(this._tooltipClass);
    this._updateTooltipMessage();
    instance.show(delay);
  }
  ...
  _createOverlay(origin) {
    ...
    strategy.positionChanges.pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe((change) => {
    ...
    this._updatePosition(this._overlayRef);
    this._overlayRef.detachments().pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe(() => this._detach());
    this._overlayRef.outsidePointerEvents().pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe(() => this._tooltipInstance?._handleBodyInteraction());
    this._overlayRef.keydownEvents().pipe((0, import_operators6.takeUntil)(this._destroyed)).subscribe((event) => {
...
var _TooltipComponent = class _TooltipComponent {
  constructor(_changeDetectorRef, _elementRef, animationMode) {
    ...
    this._onHide = new import_rxjs7.Subject();
...
var _MatPaginatorIntl = class _MatPaginatorIntl {
  constructor() {
    this.changes = new import_rxjs8.Subject();
...
var _MatPaginator = class _MatPaginator {
  ...
  constructor(_intl, _changeDetectorRef, defaults2) {
    ...
    this._isInitialized = false;
    this._initializedStream = new import_rxjs8.ReplaySubject(1);
    this._pageIndex = 0;
    this._length = 0;
